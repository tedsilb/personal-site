 ...
 * @param {{minFontSize: string, maxFontSize: string}} options Options for fitting text.
 */
export function fitText(element, options) {
  const compressor = options.compressor || 1;
  const minFontSize = parseFloat(options.minFontSize) || Number.NEGATIVE_INFINITY;
  const maxFontSize = parseFloat(options.maxFontSize) || Number.POSITIVE_INFINITY;
