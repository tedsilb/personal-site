...
$.flexslider =
  ...
  var namespace = slider.vars.namespace,
      ...
      slider.vars.touch,
      // depricating this idea, as devices are being released with both of these
      // events
      // eventType = (touch) ? "touchend" : "click",
      eventType = 'click touchend MSPointerUp', watchedEvent = '',
  ...
  methods = {
    init: function() {
      ...
      slider.currentSlide =
          parseInt((slider.vars.startAt ? slider.vars.startAt : 0));
      ...
      // TOUCH/USECSS:
      slider.transitions =
          !slider.vars.video && !fade && slider.vars.useCSS && (function() {
      ...
      if (slider.vars.manualControls !== '')
        slider.manualControls = $(slider.vars.manualControls).length > 0 &&
        ...
        if (!msGesture) {
          slider.slides.click(function(e) {
            ...
            var posFromLeft = $slide.offset().left -
                $(slider).scrollLeft();  // Find position of slide relative to
                                         // left of slider container
            if (posFromLeft <= 0 &&
              ...
              slider.flexAnimate(
                  target, slider.vars.pauseOnAction, false, true, true);
            }
          });
        } else {
          el._slider = slider;
    ...
    controlNav: {
      ...
      setupPaging: function() {
        ...
        slider.controlNavScaffold =
            $('<ol class="' + namespace + 'control-nav ' + namespace + type +
              '"></ol>');
        ...
        if (slider.pagingCount > 1) {
          for (var i = 0; i < slider.pagingCount; i++) {
            ...
            item = (slider.vars.controlNav === 'thumbnails') ?
                ...
                '<a>' + j + '</a>';
            if ('thumbnails' === slider.vars.controlNav &&
                true === slider.vars.thumbCaptions) {
              var captn = slide.attr('data-thumbcaption');
              if ('' != captn && undefined != captn)
                item += '<span class="' + namespace + 'caption">' + captn +
                    '</span>';
            }
    ...
    directionNav: {
      setup: function() {
        var directionNavScaffold =
            $('<ul class="' + namespace + 'direction-nav"><li><a class="' +
              namespace + 'prev" href="#">' + slider.vars.prevText +
              '</a></li><li><a class="' + namespace + 'next" href="#">' +
        ...
        if (slider.controlsContainer) {
          ...
          slider.directionNav = $(
              '.' + namespace + 'direction-nav li a', slider.controlsContainer);
        } else {
    ...
    pausePlay: {
      setup: function() {
        ...
        if (slider.controlsContainer) {
          ...
          slider.pausePlay =
              $('.' + namespace + 'pauseplay a', slider.controlsContainer);
        } else {
      ...
      update: function(state) {
        (state === 'play') ? slider.pausePlay.removeClass(namespace + 'pause')
        ...
        function onTouchStart(e) {
          if (slider.animating) {
            e.preventDefault();
              ...
              (window.navigator.msPointerEnabled) || e.touches.length === 1) {
            slider.pause();
            // CAROUSEL:
        ...
        function onTouchEnd(e) {
          ...
          if (slider.animatingTo === slider.currentSlide && !scrolling &&
            ...
            if (slider.canAdvance(target) &&
              ...
              if (!fade)
                slider.flexAnimate(
                    slider.currentSlide, slider.vars.pauseOnAction, true);
            }
          }
          ...
          document.addEventListener(evtname, function() {
            if (methods.pauseInvisible.isHidden()) {
              if (slider.startTimeout)
                clearTimeout(
                    slider
                        .startTimeout);  // If clock is ticking, stop timer and
                                         // prevent from starting while invisible
              else
              ...
              if (slider.started)
                ...
                (slider.vars.initDelay > 0) ?
                    setTimeout(slider.play, slider.vars.initDelay) :
                    slider.play();  // Didn't init before: simply init or wait
                                    // for it
            }
          });
        }
    ...
    setToClearWatchedEvent: function() {
      ...
      watchedEventClearTimer = setTimeout(function() {
        watchedEvent = '';
            ...
            // public methods
            slider.flexAnimate =
      function(target, pause, override, withSync, fromNav) {
      ...
      if (!fade) {
        ...
        if (carousel) {
          ...
          slideString = (calcNext > slider.limit && slider.visible !== 1) ?
            ...
            slider.currentSlide === 0 && target === slider.count - 1 &&
            slider.vars.animationLoop && slider.direction !== 'next') {
          slideString =
              (reverse) ? (slider.count + slider.cloneOffset) * dimension : 0;
        } else if (
            slider.currentSlide === slider.last && target === 0 &&
        ...
        if (slider.transitions) {
          ...
          slider.container.animate(
              slider.args, slider.vars.animationSpeed, slider.vars.easing,
              function() {
                slider.wrapup(dimension);
              });
        }
        ...
        if (!touch) {
          ...
          slider.slides.eq(slider.currentSlide)
              .css({'zIndex': 1})
              .animate(
                  {'opacity': 0}, slider.vars.animationSpeed,
                  slider.vars.easing);
          slider.slides.eq(target)
              .css({'zIndex': 2})
              .animate(
                  {'opacity': 1}, slider.vars.animationSpeed,
                  slider.vars.easing, slider.wrapup);
    ...
    if (!fade && !carousel) {
      if (slider.currentSlide === 0 && slider.animatingTo === slider.last &&
        ...
        slider.setProps(dimension, 'jumpEnd');
          ...
          slider.currentSlide === slider.last && slider.animatingTo === 0 &&
          slider.vars.animationLoop) {
    ...
    return (fromNav) ?
        ...
        (asNav && slider.currentItem === slider.count - 1 && target === 0 &&
         slider.direction === 'prev') ?
        true :
        (asNav && slider.currentItem === 0 &&
         target === slider.pagingCount - 1 && slider.direction !== 'next') ?
        false :
    ...
    if (!fade) {
      ...
      if (slider.vars.animationLoop && !carousel) {
        ...
        slider.container
            .append(slider.slides.first().clone().addClass('clone').attr(
                'aria-hidden', 'true'))
            .prepend(slider.slides.last().clone().addClass('clone').attr(
                'aria-hidden', 'true'));
      }
      ...
      if (vertical && !carousel) {
        ...
        setTimeout(function() {
          ...
          slider.newSlides.css(
              {'width': slider.computedW, 'float': 'left', 'display': 'block'});
          // SMOOTH HEIGHT:
          if (slider.vars.smoothHeight) methods.smoothHeight();
        }, (type === 'init') ? 100 : 0);
      }
...
$.flexslider.defaults = {
  namespace: 'flex-',  //{NEW} String: Prefix string attached to the class of
                       //every element generated by the plugin
  selector: '.slides > li',  //{NEW} Selector: Must match a simple pattern.
                             //'{container} > {slide}' -- Ignore pattern at your
                             //own peril
  ...
  animationLoop:
      true,  // Boolean: Should the animation loop? If false, directionNav will
             // received "disable" classes at either end
  smoothHeight: false,  //{NEW} Boolean: Allow height of the slider to animate
                        //smoothly in horizontal mode
  startAt: 0,  // Integer: The slide that the slider should start on. Array
               // notation (0 = first slide)
  slideshow: true,  // Boolean: Animate slider automatically
  slideshowSpeed:
      7000,  // Integer: Set the speed of the slideshow cycling, in milliseconds
  animationSpeed: 600,  // Integer: Set the speed of animations, in milliseconds
  initDelay: 0,  //{NEW} Integer: Set an initialization delay, in milliseconds
  randomize: false,      // Boolean: Randomize slide order
  thumbCaptions: false,  // Boolean: Whether or not to put captions on
                         // thumbnails when using the "thumbnails" controlNav.

  // Usability features
  pauseOnAction: true,  // Boolean: Pause the slideshow when interacting with
                        // control elements, highly recommended.
  pauseOnHover: false,  // Boolean: Pause the slideshow when hovering over
                        // slider, then resume when no longer hovering
  pauseInvisible:
      true,  //{NEW} Boolean: Pause the slideshow when tab is invisible, resume
             //when visible. Provides better UX, lower CPU usage.
  useCSS: true,  //{NEW} Boolean: Slider will use CSS3 transitions if available
  touch: true,   //{NEW} Boolean: Allow touch swipe navigation of the slider on
                 //touch-enabled devices
  video: false,  //{NEW} Boolean: If using video in the slider, will prevent
                 //CSS3 3D Transforms to avoid graphical glitches
  ...
  prevText:
      'Previous',  // String: Set the text for the "previous" directionNav item
  nextText: 'Next',  // String: Set the text for the "next" directionNav item

  // Secondary Navigation
  keyboard:
      true,  // Boolean: Allow slider navigating via keyboard left/right keys
  multipleKeyboard:
      false,  //{NEW} Boolean: Allow keyboard navigation to affect multiple
              //sliders. Default behavior cuts out keyboard navigation with more
  ...
  // Special properties
  controlsContainer: '',  //{UPDATED} jQuery Object/Selector: Declare which
                          //container the navigation elements should be appended
                          //too. Default container is the FlexSlider element.
                          //Example use would be $(".flexslider-container").
                          //Property is ignored if given element is not found.
  manualControls:
      '',  //{UPDATED} jQuery Object/Selector: Declare custom control
           //navigation. Examples would be $(".flex-control-nav li") or
           //"#tabs-nav li img", etc. The number of elements in your controlNav
  ...
  asNavFor: '',  //{NEW} Selector: Internal property exposed for turning the
                 //slider into a thumbnail navigation for another slider

  // Carousel Options
  itemWidth: 0,   //{NEW} Integer: Box-model width of individual carousel items,
                  //including horizontal borders and padding.
  itemMargin: 0,  //{NEW} Integer: Margin between carousel items.
  minItems: 1,  //{NEW} Integer: Minimum number of carousel items that should be
                //visible. Items will resize fluidly when below this.
  maxItems: 0,  //{NEW} Integer: Maxmimum number of carousel items that should
                //be visible. Items will resize fluidly when above this limit.
  ...
  // Callback API
  start: function() {},  // Callback: function(slider) - Fires when the slider
                         // loads the first slide
  before: function() {},  // Callback: function(slider) - Fires asynchronously
                          // with each slider animation
